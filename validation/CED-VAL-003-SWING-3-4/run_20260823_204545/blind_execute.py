"""Blind out-of-sample execution of frozen H02 on CED-VAL-003-SWING-3-4."""
...
DATASET_MANIFEST = ROOT / "validation/CED-VAL-003-SWING-3-4/input_authority_manifest.json"
SOURCES = (
    ("Drums", EXTERNAL / "steams/CED-VAL-003-SWING-3-4_drums.wav", 155, "TEMPORAL_REFERENCE", "11bd51037126608d7052ae0bb2b01d77b86eccae46d60ca088d3d5f57cccc44d"),
    ("Double Bass", EXTERNAL / "steams/CED-VAL-003-SWING-3-4_bass.wav", 100, "ACCOMPANIMENT", "bd702128f0b6e9887ccfae104ee0af6b2b4307c2021bb826fd85fec669322429"),
    ("Piano", EXTERNAL / "steams/CED-VAL-003-SWING-3-4_piano.wav", 50, "ACCOMPANIMENT", "64b95f5c41bb2bc102c68ffb2fa9b0215a2397e749f671ba2891378533302065"),
)
EXPECTED = {
    "preregistration_sha256": "10f4f445b257a42e0bdb7cd98277ebbd6689c0f76315c04ca115b0f875e50784",
    "dataset_manifest_sha256": "f53ce38c5324981753310736e47dd2620364e9a1a71848af50b4d5fb35d5e085",
...
def build_once() -> dict:
    ...
    localizations = DrumRelativeEMELocalizationBuilder().build(populations["Double Bass"] + populations["Piano"], populations["Drums"], all_candidates, temporal_origin_seconds=0.0, analysis_execution_id="H02-CEDVAL003-BLIND")
    assignments = []
    for name, _path, _count, role, _sha in SOURCES:
        sample = populations[name][0]
        assignments.append(AnalyticalRoleAssignment(assignment_id=uuid5(NAMESPACE_URL, f"CED-VAL-003-SWING-3-4-controlled-role:{name}"), source_id=sample.sound_source_id, asset_id=sample.source_asset_sha256, temporal_scope=sample.temporal_scope, temporal_origin_seconds=0.0, role=role, assignment_rule="pi-authorized-controlled-role/v1", execution_id="H02-CEDVAL003-BLIND", scientific_authority_id="AD-040", scientific_authority_fingerprint="b8983e8"))
    profile = RhythmSectionTimingProfileBuilder().build(all_events, localizations, assignments, temporal_scope=all_events[0].temporal_scope, temporal_origin_seconds=0.0, execution_id="H02-CEDVAL003-BLIND", provenance_id="PR-CED-VAL-003-SWING-3-4-001", scientific_authority_ids=("AD-037", "AD-038", "AD-040"))
...
def main() -> None:
    ...
    blind_manifest = {"experiment_id": "H-VAL001-RHYTHM-CORRESPONDENCE-02", "validation_dataset": "PR-CED-VAL-003-SWING-3-4-001", "phase": "BLIND_FROZEN", "blind_result_sha256": digest(output), "blind_scientific_fingerprint": fingerprint, "ground_truth_accessed": False, "calibration_values_consumed": False, "deterministic_replay": True}
