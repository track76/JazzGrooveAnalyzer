...
def authenticate_real(authorization_path, destination):
    ...
    binding_path = HERE/'implementation_binding.json'
...
def compare_replay(first, second, destination):
...
def main():
    ...
    if args.worker == 'generator':
        ...
        for worker in ('generator', 'checker'):
            subprocess.run([sys.executable, '-B', str(Path(__file__).resolve()), args.mode,
                args.input, str(destination), '--worker', worker], check=True)
