...
class WitnessTests(unittest.TestCase):
    ...
    def fresh(self, values=(0, 1, 2, 3)):
        self.serial += 1
        fixture_path = self.root/f'input{self.serial}.json'
        fixture_path.write_bytes(canonical(fixture(list(values)))+b'\n')
        dest = self.root/f'fresh{self.serial}'
        subprocess.run([sys.executable, '-B', str(Path(__file__).with_name('runner.py')),
            'synthetic', str(fixture_path), str(dest), '--execution-authority',
            str(self.root/'ledger')], check=True, capture_output=True)
        return dest

    def compare(self, first, second):
        return runner.compare_replay(first, second, self.root/'review_replay.json',
                                     authority_directory=self.root/'ledger')
    ...
    def test_replay_mismatch_withholds_admissibility(self):
        first = self.fresh((0, 1, 2))
        second = self.fresh((0, 1, 3))
        with self.assertRaises(ValueError):
            self.compare(first, second)

    def test_SAME_PATH_SELF_COMPARISON(self):
        first = self.fresh()
        with self.assertRaises(ValueError):
            self.compare(first, first)

    def test_RESOLVED_PATH_ALIAS_SELF_COMPARISON(self):
        first = self.fresh()
        alias = self.root/'alias'
        alias.symlink_to(first, target_is_directory=True)
        with self.assertRaises(ValueError):
            self.compare(first, alias)

    def test_SAME_EXECUTION_ID_DIFFERENT_PATH(self):
        first = self.fresh()
        second = self.root/'copied'
        shutil.copytree(first, second)
        with self.assertRaises(ValueError):
            self.compare(first, second)

    def test_ONE_EXECUTION_ONLY(self):
        first = self.fresh()
        with self.assertRaises(FileNotFoundError):
            self.compare(first, self.root/'absent')
        self.assertFalse((self.root/'review_replay.json').exists())

    def test_TWO_DISTINCT_EXECUTIONS_IDENTICAL_OUTPUT(self):
        first, second = self.fresh(), self.fresh()
        self.assertNotEqual(read(first/'execution.json')['execution_id'],
                            read(second/'execution.json')['execution_id'])
        self.assertEqual(self.compare(first, second)['candidate_acceptance'],
                         'ADMISSIBLE_COMPLETE_POPULATION')

    def test_TWO_DISTINCT_EXECUTIONS_DIFFERENT_OUTPUT(self):
        first, second = self.fresh(), self.fresh()
        path = second/'relations.jsonl'
        path.write_bytes(path.read_bytes()+b'\n')
        with self.assertRaises(ValueError):
            self.compare(first, second)
        self.assertFalse((self.root/'review_replay.json').exists())

    def test_TAMPERED_RUN_IDENTITY(self):
        first, second = self.fresh(), self.fresh()
        receipt = read(second/'execution.json')
        receipt['workers'][0]['launch_id'] = '0'*32
        (second/'execution.json').write_bytes(canonical(receipt)+b'\n')
        with self.assertRaises(ValueError):
            self.compare(first, second)

    def test_COPIED_RUN_PRESENTED_AS_FRESH_EXECUTION(self):
        first = self.fresh()
        second = self.root/'relabelled'
        shutil.copytree(first, second)
        receipt = read(second/'execution.json')
        receipt['execution_id'] = '0'*32
        receipt['directory']['resolved_path'] = str(second)
        receipt['directory']['inode'] = str(second.stat().st_ino)
        (second/'execution.json').write_bytes(canonical(receipt)+b'\n')
        with self.assertRaises(FileNotFoundError):
            self.compare(first, second)
