...
def main():
 ...
 p=json.load(open(HERE/'protocol.json'));q=dict(p);fp=q.pop('protocol_fingerprint');assert hashlib.sha256(canonical(q)).hexdigest()==fp;auth=p['authorities_locked_until_evaluation'];assert sha(auth['correspondence']['path'])==auth['correspondence']['sha256'];assert sha(auth['original_bass_audio']['path'])==auth['original_bass_audio']['sha256']
 cand=json.load(open(a.candidates));score=json.load(open(auth['correspondence']['path']))['runs']['M1_run_1']['level_2']['Double Bass']; originals=[]
 ...
 ncan=len(cand['candidates']);nmatch=len(matches);new=sum(m['baseline_status']=='MISSED_BASS' for m in matches);old=nmatch-new;unmatched=ncan-nmatch;precision=nmatch/ncan if ncan else 0;missrec=new/436;incr=new/1055;combined=(619+new)/1055;allrec=nmatch/1055;f1=2*precision*allrec/(precision+allrec) if precision+allrec else 0;disp=[m['absolute_displacement_seconds'] for m in matches]
