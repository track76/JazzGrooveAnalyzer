...
Dependency order remains observation → translation/evidence provenance → operational representation → interpretation. Original audio feeds PLP, native observability and authorized separation; source hypotheses do not rewrite Core observations. See the PI decision for the architecture diagram and exact timestamp/DUAL/matching rules.
...
[Documentation-level research decision](architecture/RESEARCH_MODULAR_GROOVE_DECISION_20260921.md).
...
**Active v1 architecture:** historical full mix → observability → independent Bass and generic-Drum inventories → internal quarter grid → marker-to-grid geometry → optional individually qualified attack evidence → musical report with MM:SS listening examples and abstentions → historical corpus. Generic Drum component identity is not required. Older physical-onset and direct Bass↔Drum paths below are future/deferred, not the current established output.


# Software Architecture

## Historical v1 product path — observable groove, 2026-09-20 (superseded above)

The [frozen PI finalization directive](scientific/rfc/JGA_FINALIZATION_DIRECTIVE_001_20260920/README.md) and [current session closure](scientific/rfc/JGA_SESSION_CLOSURE_OBSERVABLE_GROOVE_20260920/README.md) define the **active product workflow**. The high-level source-separation pipeline later in this document remains historical/general software architecture and is not a prerequisite for this explicitly authorized original-full-mix demonstration.

```text
Historical full mix → observability (UNKNOWN is valid)
→ independent Bass and generic-Drum event inventories
→ internal quarter-pulse grid
→ same-quarter musical correspondence
→ earliest defensible source-associated attacks on original audio
→ Δt = t_DRUM − t_BASS (ms)
→ observed distribution → scatter + MM:SS listening examples
→ musical/historical report → historical corpus.
```

The frozen inventories contain 496 Bass and 500 generic-Drum HIGH+MEDIUM candidates (inventory freeze SHA-256 `a76c0533d5e3e7e7572a593cfc2b762fa5bf418ea13cf3e69425afbc74fd34e6`). The frozen grid (SHA-256 `18e5d3283f9de9b878fbfe4e956805717e86a55da91a9c7c2494ca595cf0941d`) has initial 164.76 BPM and median local 164.09 BPM. PI manual tap ~162 BPM is approximate; ~120 BPM belongs to “Easy Does It.” Automatic phase audit remains PARTIALLY_SUPPORTED; subsequent PI click listening confirms quarter pulse through 01:55–02:10 despite reduced acoustic support of unknown cause. Same-quarter candidates (127 opportunities, 118 single-candidate, nine ambiguous) are not performer timing. Positive offset means Drum after Bass; negative means Drum before Bass. Triplets/subdivisions must not redefine the quarter. Do not compare late Bass low-frequency maxima to early Drum spectral maxima as performer timing. Preserve source and onset uncertainty. Ride/Hi-Hat component identity, bar downbeat, source separation, and universal full-recording recognition are not gates for the next generic-Drum result. **Next milestone: `JGA-BASS-DRUM-MICROTIMING-001`.**
...
# Historical / General Software Pipeline
