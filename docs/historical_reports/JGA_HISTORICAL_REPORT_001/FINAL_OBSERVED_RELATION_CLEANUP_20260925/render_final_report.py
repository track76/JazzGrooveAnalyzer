...
A=O.parent/'FINAL_ANCHORED_BMIG_DMIG_20260925'
preflight()
...
dis=[r for r in rows(O/'QUARTER_RELATIONS.csv') if r['displayed']=='YES'];ev=rows(A/'EVENT_PROVENANCE_127.csv')
refs=rows(A/'source_visual/METRIC_REFERENCE.csv');tt=[float(r['reference_time']) for r in refs];win=rows(A/'source_visual/BPM_EVOLUTION.csv');central=json.loads((A/'source_visual/PROFILE.json').read_text())['tempo']['central_bpm']
reader_text='Nella selezione analizzata, le pulsazioni di contrabbasso e batteria risultano complessivamente ravvicinate. Nei 43 quarti con attacchi osservati di entrambi gli strumenti, la differenza mediana tra le griglie è +16,9 ms, con il contrabbasso leggermente arretrato. La presenza di anticipi e ritardi non indica una posizione uniformemente avanzata o arretrata.'
bpm_text='BPM interno: 160,2 → 167,0 · incremento +6,8 BPM'
plt.rcParams.update({'font.size':7,'axes.spines.top':False,'axes.spines.right':False,'font.family':'DejaVu Sans'})
blue='#286ea8';orange='#c66a16';rendered=[];gridlog=[];squarelog=[];localbp=[];bpmcheck=[]
mem=io.BytesIO()
with PdfPages(mem) as pdf:
 ...
 fig.text(.07,.939,'BPM interno medio: 164,5 · Variazione BPM: +6,8 BPM · Contrabbasso–Batteria: mediana +16,9 ms, Bass dietro',fontsize=7)
 ...
 selected=json.loads((A/'source_visual/BPM_EXTREMA_ANNOTATIONS.json').read_text())['selected']
 boxes=[]
 for c in selected:
 ...
 for local,start in enumerate([130,146,162,178]):
  ...
  for r in dis:
   ...
   for inst,prefix,color,width,style,z in [('Bass','B',blue,1.15,'-',2),('Drum','D',orange,1.5,(0,(2.1,1.7)),3)]:
    ...
    ax.annotate(lab,(tx,ly),xytext=(2,0),textcoords='offset points',fontsize=5.5,color='#777777' if r['square_marker']=='YES' else color,va='center',annotation_clip=False)
    gridlog.append({'instrument':inst,'quarter':r['quarter_ID'],'native_grid_time_s':str(tx),'render_x_s':str(tx)})
   if r['square_marker']=='YES':
    ax.scatter([float(r['DMIG_time_s'])],[.5],marker='s',s=14,facecolors='white',edgecolors='#555555',linewidths=.7,zorder=9,clip_on=False)
    squarelog.append({'quarter_ID':r['quarter_ID'],'x_s':r['DMIG_time_s'],'primary_included':'NO'})
  for e in ev:
 ...
 handles.append(Line2D([],[],marker='s',mfc='white',mec='#555555',ls='',label='Ricostruito'))
 fig.legend(handles=handles,loc='lower center',bbox_to_anchor=(.52,.023),ncol=8,fontsize=6,frameon=False,handlelength=1.4,columnspacing=1)
 fig.text(.08,.012,'BMIG = Bass Measure Internal Grid · DMIG = Drum Measure Internal Grid',fontsize=6)
 pdf.savefig(fig);fig.savefig('/private/tmp/jga_relation_final_page1.png',dpi=200);plt.close(fig)
 fig=plt.figure(figsize=(210/25.4,297/25.4))
 ...
 pdf.savefig(fig);fig.savefig('/private/tmp/jga_relation_final_page2.png',dpi=200);plt.close(fig)
assert len(rendered)==127 and len({e['event_ID'] for e in rendered})==127
assert len(gridlog)==94 and len(squarelog)==4
put('JGA_FINAL_REPORT.pdf',mem.getvalue());table('RENDERED_EVENTS.csv',rendered);table('RENDERED_GRIDS.csv',gridlog);table('RENDERED_SQUARES.csv',squarelog)
