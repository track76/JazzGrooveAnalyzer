...
**Current pipeline boundary:** full mix → observability → independent Bass and generic-Drum inventories → frozen quarter grid → marker-to-quarter report; source-associated attack qualification only when individually bounded or prospectively validated. No performer Bass↔Drum offset is established for a broad passage.


## Historical v1 product pipeline — 2026-09-20 (superseded above)

**Goal:** complete JGA v1 as an observable groove analyzer. **Next:** `JGA-BASS-DRUM-MICROTIMING-001` on “Exactly Like You,” after separate PI authorization. Current frozen inputs: 496 Bass and 500 generic-Drum HIGH+MEDIUM candidates (inventory freeze `a76c0533d5e3e7e7572a593cfc2b762fa5bf418ea13cf3e69425afbc74fd34e6`), unchanged quarter grid (freeze `18e5d3283f9de9b878fbfe4e956805717e86a55da91a9c7c2494ca595cf0941d`), and 127 same-quarter opportunities (118 single-candidate, nine ambiguous). PI click listening confirms quarter pulse in 01:55–02:10; automatic support remains PARTIALLY_SUPPORTED.

```text
Historical full mix → observability → independent Bass and generic-Drum
event inventories + internal quarter-pulse grid → same-quarter musical
correspondence → source-associated attack refinement on original audio
→ Δt = t_DRUM − t_BASS (ms) → observed timing distribution
→ scatter graph + MM:SS listening examples → musical/historical report
→ historical corpus.
```

Positive Δt means Drum after Bass; negative means Drum before Bass. Triplets/subdivisions stay inside the quarter and must not redefine BPM or phase. Use only defensibly observable events; UNKNOWN is valid. Preserve event grade and onset uncertainty. A late Bass low-frequency maximum and early Drum spectral maximum must not be compared as performer timing. Ride/Hi-Hat classification, bar downbeat, universal recognition, model search/training, Gallegati/Virtual Tap development, Rubner/Yamaha, and source separation are not blockers. The generated layer diagram below is historical software context, not the active research route.
